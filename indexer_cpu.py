...
def quat_mul(q, p):
    ...
    Args:
        q: First quaternion (w, x, y, z)
        p: Second quaternion (w, x, y, z)
...
def quat_geodesic_angle(q1, q2):
    """Compute geodesic angle between two quaternions in radians.
    ...
    Returns:
        Geodesic angle in radians (0 to π)
    """
    q1 = quat_normalize(q1)
    q2 = quat_normalize(q2)
    d = np.abs(float(np.dot(q1, q2)))
    d = max(min(d, 1.0), -1.0)
    return 2.0 * np.arccos(d)

def quat_small_perturb(q, theta_rad):
    """Apply a small random rotation to a quaternion.
    
    Args:
        q: Input quaternion
        theta_rad: Maximum rotation angle in radians
        
    Returns:
        Perturbed quaternion
    """
    if theta_rad <= 0:
    ...
    half = 0.5 * theta_rad
    p = np.array([np.cos(half), *(np.sin(half) * u)], dtype=np.float64)
    return quat_normalize(quat_mul(p, q))

def canonicalize_by_symmetry(q, rot_mats, ref_R=None):
    """Find canonical quaternion representation under crystal symmetry operations.
    
    Applies all symmetry operations to the quaternion and selects the one that
    maximizes trace (or alignment with reference if provided).
    
    Args:
        q: Input quaternion
        rot_mats: Array of symmetry rotation matrices
        ref_R: Optional reference rotation matrix for alignment
    ...
    if ref_R is None:
        ...
        for g in rot_mats:
            cand = Rq @ g
            tr = float(np.trace(ref_R.T @ cand))
            if tr > best_score:
                best_score = tr
                best_R = cand
    t = np.trace(best_R)
    if t > 0:
        S = np.sqrt(t + 1.0) * 2.0
        q0 = 0.25 * S
        q1 = (best_R[2, 1] - best_R[1, 2]) / S
        q2 = (best_R[0, 2] - best_R[2, 0]) / S
        q3 = (best_R[1, 0] - best_R[0, 1]) / S
    else:
        i = int(np.argmax([best_R[0, 0], best_R[1, 1], best_R[2, 2]]))
        if i == 0:
            S = np.sqrt(1.0 + best_R[0, 0] - best_R[1, 1] - best_R[2, 2]) * 2.0
            q0 = (best_R[2, 1] - best_R[1, 2]) / S
            q1 = 0.25 * S
            q2 = (best_R[0, 1] + best_R[1, 0]) / S
            q3 = (best_R[0, 2] + best_R[2, 0]) / S
        elif i == 1:
            S = np.sqrt(1.0 + best_R[1, 1] - best_R[0, 0] - best_R[2, 2]) * 2.0
            q0 = (best_R[0, 2] - best_R[2, 0]) / S
            q1 = (best_R[0, 1] + best_R[1, 0]) / S
            q2 = 0.25 * S
            q3 = (best_R[1, 2] + best_R[2, 1]) / S
        else:
            S = np.sqrt(1.0 + best_R[2, 2] - best_R[0, 0] - best_R[1, 1]) * 2.0
            q0 = (best_R[1, 0] - best_R[0, 1]) / S
            q1 = (best_R[0, 2] + best_R[2, 0]) / S
            q2 = (best_R[1, 2] + best_R[2, 1]) / S
            q3 = 0.25 * S
    return quat_normalize(np.array([q0, q1, q2, q3], dtype=np.float64))

def niche_cluster_quats(quats, rot_mats, radius_deg=4.0):
    """Cluster quaternions by geodesic distance using symmetry-aware canonicalization.
    
    Groups quaternions within a specified angular radius after applying symmetry
    canonicalization. Used for diversity preservation in optimization.
    
    Args:
        quats: List of quaternions to cluster
        rot_mats: Symmetry rotation matrices
        radius_deg: Clustering radius in degrees
        
    Returns:
        Tuple of (clusters, centers) where clusters is a list of index lists
        and centers is a list of canonical cluster center quaternions
    """
    n = len(quats)
    if n == 0:
        return [], []
    q_canon = np.array([canonicalize_by_symmetry(quats[i], rot_mats) for i in range(n)])
    assigned = np.full(n, -1, dtype=int)
    centers, clusters = [], []
    rad = np.deg2rad(radius_deg)
    for i in range(n):
        if assigned[i] >= 0:
            continue
        cid = len(centers)
        centers.append(q_canon[i])
        clusters.append([i])
        assigned[i] = cid
        for j in range(i + 1, n):
            if assigned[j] >= 0:
                continue
            if quat_geodesic_angle(q_canon[j], centers[cid]) <= rad:
    ...
    for cl in clusters:
        Q = q_canon[cl]
        q0 = Q[0]
        signs = np.sign(np.sum(Q * q0, axis=1))
        Q_aligned = Q * signs[:, None]
        qc = quat_normalize(np.mean(Q_aligned, axis=0))
        new_centers.append(qc)
...
def _robust_scale_from_norms_maxwell(r, eps=1e-12):
    """Estimate scale parameter from residual norms assuming Maxwell distribution.
    
    Args:
        r: Array of residual norms
